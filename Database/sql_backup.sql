...
CREATE OR REPLACE FUNCTION gosha."fnCalculation"(
	...
	RETURNS TABLE(Height numeric, WindTemp numeric, Wind numeric, Bullet numeric(8,2))
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE PARALLEL UNSAFE
AS $BODY$
begin
	...
	unnest(gosha."prGetWindTempDelta"(gosha."fnHeaderGetTemperature"(temperature))) as WindTemp, 
	unnest(gosha."fnGetAvgWind"(wind_speed, device_name)) as Wind, 
	unnest(gosha."fnGetBullet"(bullet_param, device_name)) as Bullet
	from headers;
end;
$BODY$;

-- 'Десантный метео комплект';
select gosha."fnCalculation"(-18, 20, 14, 'Десантный метео комплект');
